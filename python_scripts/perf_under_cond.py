...
def get_perf_under_cond() -> pd.DataFrame:
    ...
    for test_repo in os.listdir("./test_repos"):
        ...
        no_mt = json.load(
            open(test_dir + "/perf_trace_no_multithreading.json"))
        base = json.load(open(test_dir + "/perf_trace.json"))
        no_defender = json.load(
            open(test_dir + "/perf_trace_no_defender.json"))
...
if __name__ == "__main__":
    os.chdir("..")
    results_df = get_perf_under_cond()
